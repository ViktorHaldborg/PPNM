			...
			{	string h = args[0];
				using (StreamReader sr = new StreamReader($"{h}"))
					...
					using (StreamWriter outputFile = new StreamWriter($"{y}"))
					{
						...
						if(linez<0){x=1-linez;double gamma=Exp(x*Log(x+1/(12*x-1/x/10))-x+Log(2*PI/x)/2);;outputFile.WriteLine($"{linez}	{PI/Sin(PI*linez)/gamma}");}
						else{x=1+linez;double gamma=Exp(x*Log(x+1/(12*x-1/x/10))-x+Log(2*PI/x)/2);;outputFile.WriteLine($"{linez}	{gamma/linez}");}
						} 
					}
