...
public static Tuple<matrix,matrix> Search(int n, int Points, int ε, int δ, int μ){ // Implemented search+ Newton Raphson search
...
for(int i=0; i<SingularityCount; i=i+1)
{
	if(o==p)
	{
		o=o+1;  // Skipping p'th element
	}
...
// Lets convert d back to a vector
...
IntHigh=IntLow+3*Max(Abs(a),Abs(b));
kk=0; 
step=(IntHigh-IntLow)/Points;
...
for(ii[0]=IntLow; ii[0]<IntHigh; ii[0]=ii[0]+step) // Displacing the search a little since we wont hit the singularity
{
	...
	if(Round(f(NewtonUpdateMatrix(f,f1,x0,ε,n))[0],δ)==0 && NewtonUpdateMatrix(f,f1,x0,ε,n)[0]>Singularity[SingularityCount-1] 
		&& Round(NewtonUpdateMatrix(f,f1,x0,ε,n)[0],μ)!=A_d[AddCounter,AddCounter])
	{
		A_d[AddCounter,AddCounter]=NewtonUpdateMatrix(f,f1,x0,ε,n)[0]; AddCounter=AddCounter+1; break;
	}
		
	if(Round(f(x0)[0],δ)==0 && x0[0]>Singularity[SingularityCount-1] && Round(NewtonUpdateMatrix(f,f1,x0,ε,n)[0],μ)!=A_d[AddCounter,AddCounter])
	{
		A_d[AddCounter,AddCounter]=x0[0]; AddCounter=AddCounter+1; break;
	}
	kk=kk+1; 
