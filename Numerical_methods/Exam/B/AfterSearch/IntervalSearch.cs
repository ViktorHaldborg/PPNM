...
int ZeroCount=n-AddCounter;
int MissCounter=0; // For indexing old unchanged values
vector MissingEigenvalues=new vector(ZeroCount); // For adding old unchanged values



if(ZeroCount!=0) // Find unchanged old eigenvalues
{
	for(int i=0; i<n; i=i+1)
	{
		if(u[i,0]==0 && i!=p)
		{
			MissingEigenvalues[MissCounter]=i;
			MissCounter++;
		}
	}
	
	double[] IntervalEigenvalues=new double[n];
	for(int i=0; i<n; i=i+1)
	{
		IntervalEigenvalues[i]=A_d[i,i]; // last (#ZeroCount) values will be zero
	}

	// Filling in the rest
	for(int i=0; i<ZeroCount; i=i+1)
	{
		IntervalEigenvalues[i+n-ZeroCount]=D[Convert.ToInt32(MissingEigenvalues[i]),Convert.ToInt32(MissingEigenvalues[i])];
	}

	Array.Sort(IntervalEigenvalues); // Finally all eigenvalues should be obtained and sorted.

	for(int i=0; i<n; i=i+1)
	{
		A_d[i,i]=IntervalEigenvalues[i]; // Store in A_d
	}
}
