...
public class JacobiDiagonalization1{

public static int JacobiDiag(matrix A, vector e, matrix V){
int sweeps=0; int n=A.size1; int q; int p; int check;
for(int i=0; i<n; i=i+1){e[i]=A[i,i];} // Diagonal of A
for(int i=0; i<n; i=i+1){for(int j=0; j<n; j=j+1){if(i==j){V[i,j]=1.0;} else{V[i,j]=0.0;}}}

do{sweeps=sweeps+1; check=0; // Break out of loop after executation if convergence criterion holds
for(p=0; p<n; p=p+1){for(q=p+1; q<n; q=q+1){
...
double a_pp1=Pow(c,2)*a_pp-2*s*c*a_pq+Pow(s,2)*a_qq; double a_qq1=Pow(s,2)*a_pp+2*s*c*a_pq+Pow(c,2)*a_qq; // Diagonal entries after the sweep
